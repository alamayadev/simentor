...
import { getSkPersonnelKind, selectSuratKeputusanDocxEndpoint } from '../lib/surat-keputusan';
import type {
  PaginatedResponse,
  SuratKeputusan,
  SkDetil,
...
type DeleteTarget =
  | { type: 'header'; item: SuratKeputusan }
  | { type: 'detail'; item: SkDetil }
  | null;
...
function createEmptyDetailForm(selected?: SuratKeputusan | null): DetailFormData {
  return {
    pegawai_id: '',
    mitra_id: '',
    penugasan_id: '',
...
function detailToForm(item: SkDetil): DetailFormData {
  return {
...
export function SuratKeputusanWorkflow() {
  ...
  const { data: formOptionsResponse } = useApiQuery(
    ['surat-keputusan-form-options'],
    () => suratKeputusanService.formOptions(),
  );

  const { data: yearsResponse } = useApiQuery(
    ['surat-keputusan-years'],
    () => suratKeputusanService.years(),
  );

  const { data: datesResponse } = useApiQuery(
    ['surat-keputusan-dates', filterYear],
    () => suratKeputusanService.dates({ tahun: filterYear || undefined }),
  );
  ...
  const {
    ...
    ['surat-keputusan', debouncedSearch, filterYear, filterDate, page],
    () =>
      suratKeputusanService.list({
        per_page: pageSize,
        page,
  ...
  const { data: selectedHeaderResponse } = useApiQuery(
    ['surat-keputusan-detail', selectedId],
    () => suratKeputusanService.get(selectedId as number),
  ...
  const { data: mitraPenugasanResponse, isLoading: isLoadingMitraPenugasan } = useApiQuery(
    ...
    () => skDetilService.mitraPenugasanOptions(selectedKegiatanId),
  ...
  const personnelKind = getSkPersonnelKind(details);
  const docxEndpoint = selectedId ? selectSuratKeputusanDocxEndpoint(selectedId, details) : null;
    ...
    () => ({
      ...
      tipe:
        personnelKind === 'organik'
          ? 'Organik'
          : personnelKind === 'mitra_penugasan'
            ? 'Mitra Penugasan'
            : personnelKind === 'mitra_manual'
              ? 'Mitra Manual'
              : personnelKind === 'mixed'
                ? 'Campuran'
                : '-',
      nomorBaru: formOptionsResponse?.data?.nomor_baru_sk ?? '-',
    }),
  ...
  const createHeaderMutation = useApiMutation(
    (payload: Record<string, unknown>) => suratKeputusanService.create(payload),
    {
      invalidateKeys: [['surat-keputusan'], ['surat-keputusan-form-options'], ['surat-keputusan-years'], ['surat-keputusan-dates']],
      onSuccess: (response) => {
        setSelectedId(response.data.id);
        setHeaderModalMode(null);
        setNotice({ tone: 'success', message: response.message || 'Surat keputusan berhasil dibuat.' });
      },
      onError: (mutationError) => {
        setNotice({ tone: 'error', message: mutationError.message || 'Gagal membuat surat keputusan.' });
      },
    },
  );

  const insertHeaderMutation = useApiMutation(
    (payload: Record<string, unknown>) => suratKeputusanService.insert(payload),
    {
      invalidateKeys: [['surat-keputusan'], ['surat-keputusan-form-options'], ['surat-keputusan-years'], ['surat-keputusan-dates']],
      onSuccess: (response) => {
        setSelectedId(response.data.id);
        setHeaderModalMode(null);
        setNotice({ tone: 'success', message: response.message || 'Surat keputusan sisip berhasil dibuat.' });
      },
      onError: (mutationError) => {
        setNotice({ tone: 'error', message: mutationError.message || 'Gagal membuat surat keputusan sisip.' });
      },
    },
  );
  ...
  const updateHeaderMutation = useApiMutation(
    ({ id, payload }: { id: number; payload: Record<string, unknown> }) =>
      suratKeputusanService.update(id, payload),
    {
      invalidateKeys: [['surat-keputusan'], ['surat-keputusan-detail'], ['surat-keputusan-dates']],
      onSuccess: (response) => {
        setHeaderModalMode(null);
        setNotice({ tone: 'success', message: response.message || 'Surat keputusan berhasil diperbarui.' });
      },
      onError: (mutationError) => {
        setNotice({ tone: 'error', message: mutationError.message || 'Gagal memperbarui surat keputusan.' });
      },
    },
  );

  const deleteHeaderMutation = useApiMutation(
    (id: number) => suratKeputusanService.delete(id),
    {
      invalidateKeys: [['surat-keputusan'], ['surat-keputusan-form-options'], ['surat-keputusan-years'], ['surat-keputusan-dates']],
      onSuccess: () => {
        setDeleteTarget(null);
        setSelectedId(null);
        setNotice({ tone: 'success', message: 'Surat keputusan berhasil dihapus.' });
      },
      onError: (mutationError) => {
        setNotice({ tone: 'error', message: mutationError.message || 'Gagal menghapus surat keputusan.' });
      },
    },
  );

  const createDetailMutation = useApiMutation(
    (payload: Record<string, unknown>) => skDetilService.create(payload),
    {
      invalidateKeys: [['sk-details', selectedId]],
      onSuccess: () => {
        setDetailModal(null);
        setNotice({ tone: 'success', message: 'Detail surat keputusan berhasil ditambahkan.' });
      },
      onError: (mutationError) => {
  ...
  const updateDetailMutation = useApiMutation(
    ({ id, payload }: { id: number; payload: Record<string, unknown> }) => skDetilService.update(id, payload),
    {
      invalidateKeys: [['sk-details', selectedId]],
      onSuccess: () => {
        setDetailModal(null);
        setNotice({ tone: 'success', message: 'Detail surat keputusan berhasil diperbarui.' });
      },
      onError: (mutationError) => {
  ...
  const bulkMitraMutation = useApiMutation(
    (payload: Record<string, unknown>) => skDetilService.bulkMitra(payload),
    {
      invalidateKeys: [['sk-details', selectedId]],
      onSuccess: (response) => {
        setDetailModal(null);
  ...
  const deleteDetailMutation = useApiMutation(
    (id: number) => skDetilService.delete(id),
    {
      invalidateKeys: [['sk-details', selectedId]],
      onSuccess: () => {
        setDeleteTarget(null);
        setNotice({ tone: 'success', message: 'Detail surat keputusan berhasil dihapus.' });
      },
      onError: (mutationError) => {
  ...
  const headerFields = useMemo<FormField[]>(() => {
    ...
    return [
      { name: 'thn', label: 'Tahun', type: 'text', required: true, placeholder: String(currentYear) },
      ...
      { name: 'kegiatan', label: 'Kegiatan', type: 'text', required: true, placeholder: 'Nama kegiatan', colSpan: 2 as const },
      { name: 'perihal', label: 'Perihal / Menimbang', type: 'textarea', placeholder: 'Perihal atau konsideran menimbang', colSpan: 2 as const },
      {
        name: 'oleh',
  ...
  const headerInitialData = useMemo(() => {
    ...
    const base = {
      thn: selectedHeader?.thn ?? filterYear ?? String(currentYear),
      tanggal: selectedHeader?.tanggal ?? new Date().toISOString().slice(0, 10),
      nomor: selectedHeader?.nomor ?? formOptionsResponse?.data?.nomor_baru_sk ?? '',
      kode_klas: selectedHeader?.kode_klas ?? '',
      kepada: selectedHeader?.kepada ?? '',
      kegiatan: selectedHeader?.kegiatan ?? '',
      perihal: selectedHeader?.perihal ?? '',
      oleh: selectedHeader?.oleh ?? 'KEPALA',
    };

    if (headerModalMode === 'create') {
      return {
        thn: filterYear || String(currentYear),
  ...
  const canAddMitra = selectedId !== null && (personnelKind === 'empty' || personnelKind === 'mitra_penugasan' || personnelKind === 'mitra_manual');

  const openDetailModal = (type: 'organik' | 'mitra', item?: SkDetil, bulk = false) => {
    setNotice(null);
    setDetailModal({
  ...
  const handleHeaderSubmit = async (raw: Record<string, unknown>) => {
    const payload: Record<string, unknown> = {
      ...
      kegiatan: toText(raw.kegiatan),
      perihal: toText(raw.perihal),
      oleh: toText(raw.oleh),
    };

    if (!payload.thn || !payload.tanggal || !payload.kode_klas || !payload.kepada || !payload.kegiatan) {
      setNotice({ tone: 'error', message: 'Tahun, tanggal, klasifikasi, kepada, dan kegiatan wajib diisi.' });
      return;
    }
  ...
  const buildDetailPayload = () => {
    ...
    if (detailModal?.type === 'organik') {
      ...
      return {
        ...
        grup_pegawai: null,
        penugasan_id: null,
        jabatan_kegiatan: toText(detailForm.jabatan_kegiatan),
        isOrganik: true,
      };
    }
  ...
  const handleRefresh = async () => {
    setNotice(null);
    await queryClient.invalidateQueries({ queryKey: ['surat-keputusan'] });
    await queryClient.invalidateQueries({ queryKey: ['sk-details'] });
    await refetch();
  ...
  const handleDownloadDocx = async () => {
    ...
    try {
      const blob = await suratKeputusanService.downloadDocx(docxEndpoint);
      downloadBlob(blob, `${sanitizeFilename(selectedHeader.no_surat || 'surat-keputusan')}.docx`);
      setNotice({ tone: 'success', message: 'DOCX surat keputusan berhasil diunduh.' });
    } catch (downloadError) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1.1fr)_minmax(380px,0.9fr)] gap-5">
        <div className="bg-white/65 dark:bg-white/[0.04] glass-strong rounded-[24px] border border-white/50 dark:border-white/10 shadow-[0_10px_40px_-15px_rgba(0,0,0,0.08)] overflow-hidden">
          {isLoading ? (
            <LoadingSkeleton variant="table" message="Memuat surat keputusan..." />
          ) : error ? (
            <ApiErrorBoundary error={error} onRetry={() => refetch()} title="Gagal Memuat Surat Tugas" />
          ) : rows.length === 0 ? (
            <EmptyState title="Belum ada surat keputusan." description="Ubah filter atau buat surat keputusan baru." />
          ) : (
            <>
                ...
                <table className="w-full text-[13px]">
                  ...
                  <tbody className="divide-y divide-gray-100/70 dark:divide-white/5">
                    {rows.map((item) => {
                      ...
                      const perihal = toText(item.perihal) || '-';
                      const kegiatan = toText(item.kegiatan) || '-';
                      return (
                              ...
                              <td colSpan={4} className="px-5 pb-5 pt-0">
                                <div className="ml-10 grid gap-3 rounded-xl border border-gray-200/70 bg-white/70 p-4 dark:border-white/10 dark:bg-white/[0.04] md:grid-cols-2">
                                  <div className="min-w-0">
                                    <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-gray-500 dark:text-gray-400">Kegiatan</p>
                                    <p className="mt-1 whitespace-pre-wrap text-[12px] leading-5 text-gray-700 dark:text-gray-300">{kegiatan}</p>
                                  </div>
                                  <div className="min-w-0">
                                    <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-gray-500 dark:text-gray-400">Perihal / Menimbang</p>
                                    <p className="mt-1 whitespace-pre-wrap text-[12px] leading-5 text-gray-700 dark:text-gray-300">{perihal}</p>
                                  </div>
                                </div>
                              </td>
                            </tr>
        ...
        >
          <div className="flex items-start justify-between gap-4 border-b border-gray-200/60 dark:border-white/10 px-5 py-4">
            <div className="min-w-0">
              ...
              <h2 className="mt-1 truncate text-base font-bold text-gray-900 dark:text-white">
                {selectedHeader?.no_surat ?? 'Pilih surat keputusan'}
              </h2>
              {selectedHeader ? (
          ...
          {!selectedId ? (
            ...
            <>
              <div 
                data-scan="aksi detail surat"
                className="flex flex-wrap gap-2 px-5 py-4 border-b border-gray-200/60 dark:border-white/10"
              >
                {canAddOrganik ? (
                  <button
              ...
              <div className="max-h-[560px] overflow-y-auto p-5">
                {isLoadingDetails ? (
                  ...
                  <div 
                    data-scan="daftar detail personel"
                    className="space-y-3"
                  >
                    {details.map((detail) => (
                      <div
                        data-scan="kartu detail personel"
                        key={detail.id}
      ...
      <ConfirmDialog
        ...
        message={
          deleteTarget?.type === 'header'
            ...
            : 'Detail petugas ini akan dihapus dari surat keputusan.'
        }
        confirmLabel="Hapus"
      ...
      <div className="relative mx-4 flex max-h-[88vh] w-full max-w-3xl flex-col rounded-2xl border border-gray-200 bg-white shadow-2xl dark:border-white/10 dark:bg-gray-900">
        ...
        >
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            {state.type === 'organik' ? (
              <TextField label="Jabatan/Kegiatan" value={form.jabatan_kegiatan} onChange={(value) => change('jabatan_kegiatan', value)} />
            ) : null}
...
export default SuratKeputusanWorkflow;
