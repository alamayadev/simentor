...
class DataSurtugTableSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        if (env('SEEDER_AUTO_TRUNCATE', false) && app()->environment('local', 'testing')) {
            try {
                \Illuminate\Support\Facades\DB::table('surat_tugas')->truncate();
                if (isset($this->command)) $this->command->info('Truncated surat_tugas');
            } catch (\Exception $e) {
                if (isset($this->command)) $this->command->warn('Could not truncate surat_tugas: ' . $e->getMessage());
            }
        }

        // Path to the JSON file
        $jsonFilePath = database_path('initial_data/surat_tugas.json');
    ...
    {
        echo "Starting streaming JSON processing for Data Surtug...\n";
        ...
        try {
            foreach ($stream as $item) {
                ...
                if (!isset($itemArray['created_at'])) $itemArray['created_at'] = now();
                if (!isset($itemArray['updated_at'])) $itemArray['updated_at'] = now();

                // Normalize each item, preserving the id field
                $normalized = SeederUtils::normalizeRow('surat_tugas', $itemArray, true);
                if (!empty($normalized)) {
            ...
            echo "Successfully seeded $processedRecords records into surat_tugas table.\n";
    ...
    {
        try {
            DB::table('surat_tugas')->insert($batchData);
        } catch (\Exception $e) {
            echo "Error inserting batch: " . $e->getMessage() . "\n";
            // Try inserting one by one as fallback
            foreach ($batchData as $row) {
                try {
                    DB::table('surat_tugas')->insert($row);
