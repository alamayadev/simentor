...
class KegiatanTableSeeder extends Seeder
{
    ...
    public function run(): void
    {
        if (env('SEEDER_AUTO_TRUNCATE', false) && app()->environment('local', 'testing')) {
            try {
                SeederUtils::truncateTable('kegiatan');
                if (isset($this->command)) $this->command->info('Truncated kegiatan');
            } catch (\Exception $e) {
                if (isset($this->command)) $this->command->warn('Could not truncate kegiatan: ' . $e->getMessage());
            }
        }

        $jsonFilePath = database_path('initial_data/kegiatan.json');
        ...
        foreach ($data as $row) {
            ...
            $normalizedRow = SeederUtils::normalizeRow('kegiatan', $row, true);

            $rows[] = $normalizedRow;
        }

        SeederUtils::insertInBatches('kegiatan', $rows);

        echo "Seeded " . count($rows) . " records into kegiatan table.\n";
    }
}
